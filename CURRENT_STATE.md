...
## Сегодня сделано: WOTD фиксы + частотность

### Частотность слов (WOTD)
- ✅ **Добавлена таблица `word_frequencies`** — 50K ивритских слов из hermitdave/FrequencyWords (OpenSubtitles 2018)
- ✅ **Сопоставление**: 85.6% словаря (6919/8083) имеют частотный ранг
- ✅ **WOTD фильтр**: freq_rank 200–10000 (золотая середина: не бытовые, не obscure)
- ✅ **Пуло**: 3894 слова подходят для WOTD (2749 с частотностью + 1145 без данных)
- ✅ Сегодняшнее слово: `זיקה` (зика́) — «связь, отношение»

### Фиксы морды
- ✅ Белые уголки WOTD-модалки убраны (border-radius с хедера)
- ✅ «Подробнее» чинится: обработка `?q=` из URL при загрузке страницы
- ✅ Грамматика: `муж./жен.` → `м.р./ж.р.`
- ✅ Склонение: 1 пример, 2–4 примера, 5+ примеров
